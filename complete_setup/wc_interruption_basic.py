...
# ---------------------------
# Mic Stream Class (with TTS interruption)
# ---------------------------
class MicStreamToSTT:
    def __init__(self, tts: SafeMicTTS, rate=16000, chunk=1024):
        self.rate = rate
        self.chunk = chunk
    ...
    def generator(self):
        while self.running:
            try:
                # Wait for up to 0.5s for a new audio chunk
                ...
                yield speech.StreamingRecognizeRequest(
                    audio_content=b'\0' * CHUNK_SIZE * 2
                )
...
def run_speech_chat_loop():
    tts = SafeMicTTS()
    mic_stream = MicStreamToSTT(tts=tts)
    llm = ChatLLM()
    ...
    try:
        while True:
            ...
            try:
                ...
                time.sleep(1)  # prevent tight restart loop
